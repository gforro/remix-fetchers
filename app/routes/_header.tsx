...
export default function Header() {
  ...
  return (
    <div className="flex flex-col gap-4 items-center">
      ...
      <span className="text-sm text-gray-500">4 seconds: {fetcher.state}</span>
      <span className="text-sm text-gray-500">6 seconds: {fetcher2.state}</span>
      <span className="text-sm text-gray-500">8 seconds: {fetcher3.state}</span>
      <div className="border px-10 py-4 border-gray-800">
        <Outlet />
      </div>
    </div>
  );
